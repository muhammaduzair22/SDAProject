...
public class FeedbackFormView extends JFrame {
  ...
  //  ArrayList<BookBus> arr=bl.returnlist();
	...
	ArrayList<FeedbackForm> arr= new ArrayList<FeedbackForm>();
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					FeedbackFormView frame = new FeedbackFormView();
	...
	{	
		...
		lblNewLabel.setBounds(127, 10, 869, 56);
		contentPane.add(lblNewLabel);
		
		JLabel lblNewLabel_1 = new JLabel("Feedbacks");
		lblNewLabel_1.setFont(new Font("Tahoma", Font.PLAIN, 20));
		lblNewLabel_1.setBounds(551, 63, 104, 38);
		...
		arr=FeedbackFormList.getInstance().GetList();
		String data [][] =new String [arr.size()][9];
		...
		for(int i=0; i<arr.size(); i++)
		{
			for(int j=0; j<9; j++)
			{
				data[i][0]=arr.get(i).getStudentName();
				data[i][1]=arr.get(i).getRollNumber();
				data[i][2]=arr.get(i).getPhoneNumber();
				data[i][3]=arr.get(i).getRouteName();
				data[i][4]=String.valueOf(arr.get(i).getBusRegNo());
				data[i][5]=arr.get(i).getDriverName();
				data[i][6]=arr.get(i).getDate();
				data[i][7]=arr.get(i).getTime();
				data[i][8]=arr.get(i).getFeedback();
			}
		}
		
		String col[]= {"Student Name","Roll Number","Phone Number","Route Name","Bus Number","Driver Name","Date","Time","Feedback"};
